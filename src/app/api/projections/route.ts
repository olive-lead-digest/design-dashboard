// /api/projections — GET all portfolio projections · POST simulate regeneration
// (tester-mode simulation of n8n Workflow 5).
...
export async function GET() {
  try {
    ...
    return NextResponse.json(getDb().projections);
  } catch (err) {
    console.error('GET /api/projections failed', err);
...
export async function POST(req: NextRequest) {
  try {
    ...
    await triggerWorkflow('projections-generator', { projection_types: requested ?? VALID_TYPES });
    ...
    console.error('POST /api/projections failed', err);
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 });
  }
}
